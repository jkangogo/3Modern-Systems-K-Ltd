...
class FlexbannersModelClient extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Client', $prefix = 'FlexbannersTable', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_flexbanners.edit.client.data', array());
	...
	{
		$table->clientname = htmlspecialchars_decode($table->clientname, ENT_QUOTES);
	}
	...
	public function validate($form, $data, $group = null)
	{
		...
		// Check for duplication of an existing client name
		if (!empty($data['clientname']))
		{
			$db = JFactory::getDBO();
			$query = $db->getQuery(true);
			$query->select('count(*)')
				->from('#__flexbannersclient')
				->where('clientname=' . $db->quote($data['clientname']))
				->where('state IN (0,1)');

			if (!empty($data['clientid']))
			{
				$query->where('clientid<>' . (int) $data['clientid']);
			}
			...
			if ($count > 0)
			{
				...
				$return = false;
				$this->setError(JText::_('ADMIN_FLEXBANNER_DUPLICATE_CLIENTNAME'));
			}
		}
